...
for i in range(0,3):
    ...
    m.refineMesh(1)
    proj = ProjectedGradient(m,[f(x) for x in m.points], [u_d(x) for x in m.points],lam,  alpha, tol = 1e-10 )
    proj.solve()
...
# for ele,triPoints in enumerate(proj.mesh.triangles):
    ...
    ax.plot_trisurf(proj.mesh.points[:,0],proj.mesh.points[:,1],proj.mesh.triangles,proj.control)
